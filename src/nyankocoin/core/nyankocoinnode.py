from decimal import Decimalqrl
...
from nyankocoin.core.OptimizedAddressState import OptimizedAddressState
...
from nyankocoin.core.misc.logger import logger
from nyankocoin.core.node import POW, SyncState
from nyankocoin.core.p2p.p2pChainManager import P2PChainManager
from nyankocoin.core.p2p.p2pPeerManager import P2PPeerManager
from nyankocoin.core.p2p.p2pTxManagement import P2PTxManagement
from nyankocoin.core.p2p.p2pfactory import P2PFactory
from nyankocoin.core.txs.CoinBase import CoinBase
...
class NyankoCoinNode:
    ...
    def monitor_chain_state(self):
        ...
        node_chain_state = nyankocoin_pb2.NodeChainState(block_number=last_block.block_number,
    ...
    def submit_send_tx(self, tx) -> bool:
        ...
        return self._p2pfactory.add_unprocessed_txn(tx, ip=None) 
    ...
    def get_ots(self,
        ...
        for page in range(page_from, max_pages + 1):
            ...
            bitfields.append(nyankocoin_pb2.OTSBitfieldByPage(ots_bitfield=bitfield, page_number=page))
    ...
    def get_mini_transactions_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = nyankocoin_pb2.GetMiniTransactionsByAddressResp()
        for tx_hash in transaction_hashes:
            mini_transaction = nyankocoin_pb2.MiniTransaction()
    ...
    def get_transactions_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        for tx_hash in transaction_hashes:
            ...
            transaction_detail = nyankocoin_pb2.GetTransactionResp(tx=tx.pbdata,
    ...
    def get_multi_sig_spend_txs_by_address(self,
        ...
        response = nyankocoin_pb2.GetMultiSigSpendTxsByAddressResp()
    ...
    def get_vote_stats(self, multi_sig_spend_tx_hash: bytes):
        ...
        return nyankocoin_pb2.GetVoteStatsResp(vote_stats=vote_stats.pbdata)
    ...
    def get_tokens_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = nyankocoin_pb2.GetTokensByAddressResp()
        for tx_hash in token_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            token_balance = self._chain_manager.get_token(address, tx.txhash)
            transaction_detail = nyankocoin_pb2.TokenDetail(token_txhash=tx.txhash,
                                                     name=tx.name,
                                                     symbol=tx.symbol,
    ...
    def get_slaves_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = nyankocoin_pb2.GetSlavesByAddressResp()
        for tx_hash in slave_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            for index in range(0, len(tx.slave_pks)):
                transaction_detail = nyankocoin_pb2.SlaveDetail(slave_address=bytes(QRLHelper.getAddress(tx.slave_pks[index])),
    ...
    def get_lattice_pks_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = nyankocoin_pb2.GetLatticePKsByAddressResp()
        for tx_hash in lattice_pk_hashes:
            tx, _ = self._chain_manager.get_tx_metadata(tx_hash)
            transaction_detail = nyankocoin_pb2.LatticePKsDetail(pk1=tx.pk1,
                                                          pk2=tx.pk2,
                                                          pk3=tx.pk3,
                                                          tx_hash=tx_hash)
    ...
    def get_multi_sig_addresses_by_address(self, address: bytes, item_per_page: int, page_number: int):
        ...
        response = nyankocoin_pb2.GetMultiSigAddressesByAddressResp()
        for multi_sig_address in multi_sig_addresses:
            multi_sig_detail = nyankocoin_pb2.MultiSigDetail(
    ...
    def get_node_info(self) -> nyankocoin_pb2.NodeInfo:
        info = nyankocoin_pb2.NodeInfo()
    ...
    def get_block_timeseries(self, block_count) -> Iterator[nyankocoin_pb2.BlockDataPoint]:
        result = []
