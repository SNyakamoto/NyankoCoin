...
from nyankocoin.core.misc import logger, db
...
class State:
    ...
    def put_fork_state(self, fork_state: nyankocoinstateinfo_pb2.ForkState, batch=None):
        self._db.put_raw(b'fork_state', fork_state.SerializeToString(), batch)

    def get_fork_state(self) -> Optional[nyankocoinstateinfo_pb2.ForkState]:
        try:
            data = self._db.get_raw(b'fork_state')
            fork_state = nyankocoinstateinfo_pb2.ForkState()
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> nyankocoin_pb2.SlaveMetadata:
        ...
        try:
            slave_metadata = nyankocoin_pb2.SlaveMetadata()
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> nyankocoin_pb2.TokenBalance:
        try:
            token_balance = nyankocoin_pb2.TokenBalance()
