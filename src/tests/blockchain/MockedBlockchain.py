...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_nyankocoin_dir


class MockedBlockchain(object):
    MAXNUMBLOCKS = 1000

    def __init__(self, nyankocoinnode, time_mock, ntp_mock):
    ...
    def create_block(self, prev_hash, mining_address=None):
        ...
        transactions = []
        block_prev = self.nyankocoinnode.get_block_from_hash(prev_hash)
        ...
        dev_config = self.nyankocoinnode._chain_manager.get_config_by_block_number(block_new.block_number)
        while not self.nyankocoinnode._chain_manager.validate_mining_nonce(blockheader=block_new.blockheader,
    ...
    def validate(self, block):
        if not block.validate(self.nyankocoinnode._chain_manager, {}):
    ...
    def add_block(self, block):
        self.validate(block)
        return self.nyankocoinnode._chain_manager.add_block(block)

    def add_new_block(self, mining_address=None):
        block_prev = self.nyankocoinnode.get_block_last()
    ...
    def create(num_blocks, mining_address=None):
        ...
        with mock.patch('nyankocoin.core.misc.ntp.getTime') as ntp_mock, \
                set_nyankocoin_dir('no_data'), \
                State() as state, \
            ...
            try:
                ...
                nyankocoinnode = NyankoCoinNode(mining_address=b'')
                nyankocoinnode.set_chain_manager(chain_manager)

                mock_blockchain = MockedBlockchain(nyankocoinnode, time_mock, ntp_mock)
