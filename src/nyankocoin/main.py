...
from nyankocoin.core.misc import ntp, logger, set_logger
from nyankocoin.core.nyankocoinnode import NyankoCoinNode
from nyankocoin.services.services import start_services
from nyankocoin.core import config
from nyankocoin.core.State import State
...
def parse_arguments():
    ...
    parser.add_argument('--miningAddress', dest='mining_address', required=False,
                        help="NyankoCoin Wallet address on which mining reward has to be credited.")
...
def main():
    ...
    logger.debug("=====================================================================================")
    logger.info("NyankoCoin Path: %s", args.nyankocoin_dir)
    ...
    nyankocoinnode = NyankoCoinNode(mining_address=mining_address)
    nyankocoinnode.set_chain_manager(chain_manager)

    set_logger.set_logger(args, nyankocoinnode.sync_state)

    #######
    # NOTE: Keep assigned to a variable or might get collected
    admin_service, grpc_service, mining_service, debug_service = start_services(nyankocoinnode)

    nyankocoinnode.start_listening()

    nyankocoinnode.start_pow(args.mining_thread_count)

    logger.info('NyankoCoin blockchain ledger %s', config.dev.version)
    if config.user.mining_enabled:
