...
from nyankocoin.core.Wallet import Wallet, WalletDecryptionError
from nyankocoin.core.misc.helper import parse_hexblob, parse_qaddress
from nyankocoin.core.MultiSigAddressState import MultiSigAddressState
...
from nyankocoin.core.txs.Transaction import Transaction
...
from nyankocoin.crypto.xmss import XMSS, hash_functions
from nyankocoin.generated import nyankocoin_pb2_grpc, nyankocoin_pb2

ENV_NYANKOCOIN_WALLET_DIR = 'ENV_NYANKOCOIN_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return nyankocoin_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = nyankocoin_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='NyankoCoin Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_NYANKOCOIN_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def nyankocoin(ctx, verbose, host, port_pub, wallet_dir, json):
    """
    NyankoCoin Command Line Interface
    """
...
@nyankocoin.command(name='wallet_ls')
@click.pass_context
def wallet_ls(ctx):
    """
...
@nyankocoin.command(name='wallet_gen')
@click.pass_context
...
@nyankocoin.command(name='wallet_add')
...
@nyankocoin.command(name='wallet_recover')
...
@nyankocoin.command(name='wallet_secret')
...
@nyankocoin.command(name='wallet_rm')
...
@nyankocoin.command(name='wallet_encrypt')
@click.pass_context
...
@nyankocoin.command(name='wallet_decrypt')
@click.pass_context
...
@nyankocoin.command(name='tx_inspect')
...
def tx_inspect(ctx, txblob):
    ...
    try:
        ...
        pbdata = nyankocoin_pb2.Transaction()
...
@nyankocoin.command(name='tx_push')
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = nyankocoin_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@nyankocoin.command(name='tx_message')
@click.option('--src', type=str, default='', prompt=True, help='signer NyankoCoin address')
@click.option('--master', type=str, default='', prompt=True, help='master NyankoCoin address')
@click.option('--addr_to', type=str, default='', prompt=True, help='NyankoCoin Address receiving this message (optional)')
...
def tx_message(ctx, src, master, addr_to, message, fee, ots_key_index):
    ...
    try:
        ...
        push_transaction_req = nyankocoin_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@nyankocoin.command(name='tx_multi_sig_create')
@click.option('--src', type=str, default='', prompt=True, help='source NyankoCoin address')
@click.option('--master', type=str, default='', prompt=True, help='master NyankoCoin address')
...
@nyankocoin.command(name='tx_multi_sig_spend')
@click.option('--src', type=str, default='', prompt=True, help='signer NyankoCoin address')
@click.option('--master', type=str, default='', help='master NyankoCoin address')
...
def tx_multi_sig_spend(ctx, src, master, multi_sig_address, dsts, amounts, expiry_block_number, fee, ots_key_index):
    ...
    try:
        ...
        # Push transaction
        print()
        print("Sending to a NyankoCoin Node...")
        stub = ctx.obj.get_stub_public_api()
        push_transaction_req = nyankocoin_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
        push_transaction_resp = stub.PushTransaction(push_transaction_req, timeout=CONNECTION_TIMEOUT)
...
@nyankocoin.command(name='tx_transfer')
@click.option('--src', type=str, default='', prompt=True, help='signer NyankoCoin address')
@click.option('--master', type=str, default='', help='master NyankoCoin address')
@click.option('--dsts', type=str, prompt=True, help='List of destination addresses')
@click.option('--amounts', type=str, prompt=True, help='List of amounts to transfer (Quanta)')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(nyankocoin_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
@nyankocoin.command(name='tx_transfertoken')
@click.option('--src', type=str, default='', prompt=True, help='source NyankoCoin address')
@click.option('--master', type=str, default='', prompt=True, help='master NyankoCoin address')
...
@nyankocoin.command(name='slave_tx_generate')
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.nyankocoin/')
    except Exception as e:
        click.echo("Unhandled error: {}".format(str(e)))
        quit(1)


@nyankocoin.command(name='token_list')
@click.option('--owner', default='', prompt=True, help='source NyankoCoin address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = nyankocoin_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = nyankocoin_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(nyankocoin_pb2.GetNodeStateReq())
