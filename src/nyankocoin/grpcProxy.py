...
from nyankocoin.generated import nyankocoin_pb2_grpc, nyankocoin_pb2, nyankocoinmining_pb2, nyankocoinmining_pb2_grpc
...
def get_addr_state(addr: bytes) -> AddressState:
    ...
    response = stub.GetAddressState(request=nyankocoin_pb2.GetAddressStateReq(address=addr))
...
def valid_payment_permission(public_stub, master_address_state, payment_xmss, json_slave_txn):
    ...
    if access_type == -1:
        ...
        public_stub.PushTransaction(request=nyankocoin_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
        return None
...
def api_proxy(api_method_name):
    ...
    :return:
    """
    stub = nyankocoin_pb2_grpc.PublicAPIStub(grpc.insecure_channel('{}:{}'.format(config.user.public_api_host,
                                                                           config.user.public_api_port),
                                                            options=[('grpc.max_receive_message_length', 10485760)]))
    public_api = nyankocoin_pb2.DESCRIPTOR.services_by_name['PublicAPI']
    api_method = public_api.FindMethodByName(api_method_name)
    api_request = getattr(nyankocoin_pb2, api_method.input_type.name)()
...
def getlastblockheader(height=0):
    ...
    request = nyankocoinmining_pb2.GetLastBlockHeaderReq(height=height)
...
def getblocktemplate(reserve_size, wallet_address):
    stub = get_mining_stub()
    request = nyankocoinmining_pb2.GetBlockToMineReq(wallet_address=wallet_address.encode())
...
def getbalance():
    stub = get_public_stub()
    grpc_response = stub.GetOptimizedAddressState(request=nyankocoin_pb2.GetAddressStateReq(address=payment_slaves[0]))
...
def getheight():
    stub = get_public_stub()
    grpc_response = stub.GetHeight(request=nyankocoin_pb2.GetHeightReq())
...
def submitblock(blob):
    stub = get_mining_stub()
    request = nyankocoinmining_pb2.SubmitMinedBlockReq(blob=bytes(hstr2bin(blob)))
...
def getblockminingcompatible(height):
    stub = get_mining_stub()
    request = nyankocoinmining_pb2.GetBlockMiningCompatibleReq(height=height)
...
def transfer(destinations, fee, mixin, unlock_time):
    ...
    response = stub.PushTransaction(request=nyankocoin_pb2.PushTransactionReq(transaction_signed=tx.pbdata))
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='NyankoCoin node')
    ...
    parser.add_argument('--network-type', dest='network_type', choices=['mainnet', 'testnet'],
                        default='mainnet', required=False, help="Runs NyankoCoin Testnet Node")
...
def main():
    ...
    mining_stub = nyankocoinmining_pb2_grpc.MiningAPIStub(grpc.insecure_channel('{0}:{1}'.format(config.user.mining_api_host,
                                                                                          config.user.mining_api_port)))
    public_stub = nyankocoin_pb2_grpc.PublicAPIStub(grpc.insecure_channel('{0}:{1}'.format(config.user.public_api_host,
                                                                                    config.user.public_api_port),
