...
from nyankocoin.daemon.helper import logger
from nyankocoin.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from nyankocoin.services.WalletAPIService import WalletAPIService
from nyankocoin.generated import nyankocoin_pb2, nyankocoin_pb2_grpc, nyankocoinwallet_pb2
from nyankocoin.generated.nyankocoinwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
from nyankocoin.crypto.xmss import XMSS

CONNECTION_TIMEOUT = 30
config.create_path(config.user.wallet_dir)
pid = os.path.join(config.user.wallet_dir, 'nyankocoin_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = nyankocoin_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = nyankocoinwallet_pb2.PlainBlockHeader()
        ...
        pblock = nyankocoinwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = nyankocoinwallet_pb2.PlainGenesisBalance()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = nyankocoin_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def sign_and_push_transaction(self,
        ...
        if push_transaction_resp.error_code != nyankocoin_pb2.PushTransactionResp.SUBMITTED:
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = nyankocoin_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = nyankocoin_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(nyankocoin_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            nyankocoin_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(nyankocoin_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(nyankocoin_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(nyankocoin_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(nyankocoin_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(nyankocoin_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(nyankocoin_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(nyankocoin_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)
    ...
    def get_node_info(self):
        return self._public_stub.GetNodeState(nyankocoin_pb2.GetNodeStateReq())
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Nyanko Coin node')
    parser.add_argument('--nyankocoindir', '-d', dest='nyankocoin_dir', default=config.user.nyankocoin_dir,
    ...
    parser.add_argument('--network-type', dest='network_type', choices=['mainnet', 'testnet'],
                        default='mainnet', required=False, help="Runs Nyanko Coin Testnet Node")
...
def main():
    ...
    nyankocoin_dir_post_fix = ''
    copy_files = []
    if args.network_type == 'testnet':
        nyankocoin_dir_post_fix = '-testnet'
    ...
    config.user.nyankocoin_dir = os.path.expanduser(os.path.normpath(args.nyankocoin_dir) + nyankocoin_dir_post_fix)
    config.create_path(config.user.nyankocoin_dir, copy_files)
    config.user.load_yaml(config.user.config_path)

    daemon = Daemonize(app="nyankocoin_walletd", pid=pid, action=run)
    daemon.start()
