...
from nyankocoin.generated import nyankocoinstateinfo_pb2
...
class LastTransactions:
    def __init__(self, pbdata=None):
        self._data = pbdata
        if not pbdata:
            self._data = nyankocoinstateinfo_pb2.LastTransactions()
    ...
    def add(self, tx: Transaction, block_number: int, timestamp: int):
        tm = nyankocoinstateinfo_pb2.TransactionMetadata(transaction=tx.pbdata,
    ...
    def deserialize(data):
        pbdata = nyankocoinstateinfo_pb2.LastTransactions()
